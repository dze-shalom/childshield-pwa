...
export default function AbuseReport() {
  ...
  if (submitted) return (
    <div className="page" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
      ...
      <div style={{ width: '100%', background: '#111827', borderRadius: 16, border: '1px solid rgba(139,92,246,0.2)', padding: 16, marginBottom: 14, textAlign: 'left' }}>
        <p style={{ fontSize: 10, color: 'rgba(241,245,249,0.5)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600, marginBottom: 10 }}>What Happens Next</p>
        {[
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
            ...
            <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.6)', lineHeight: 1.5, margin: 0 }}>{text}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ width: '100%', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 14, padding: 14, marginBottom: 14, textAlign: 'left' }}>
        ...
        {[
          ...
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: 'rgba(241,245,249,0.5)' }}>{c.name}</span>
  ...
  return (
    <div className="page">
      ...
      {step === 'landing' && (
        <div>
          ...
          <div style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.18)', borderRadius: 14, padding: 14, marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.5)', marginBottom: 12, lineHeight: 1.5 }}>Call police immediately. Go to a public place where people can see you.</p>
          ...
          {[
            ...
            <button key={opt.id} onClick={() => setStep(opt.id)}
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontWeight: 700, fontSize: 13, color: '#F1F5F9', margin: '0 0 2px' }}>{opt.label}</p>
                <p style={{ fontSize: 11, color: 'rgba(241,245,249,0.45)', margin: 0 }}>{opt.desc}</p>
              </div>
      ...
      {step === 'who' && (
        <div>
          <h2 style={{ color: '#F1F5F9', fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Who is making this report?</h2>
          <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.5)', marginBottom: 16, lineHeight: 1.5 }}>Your answer helps us guide you to the right support. You remain completely anonymous.</p>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
            {WHO_OPTIONS.map(opt => (
              <button key={opt.id} onClick={() => setWho(opt.id)}
                style={{ background: who === opt.id ? opt.bg : '#111827', border: `1px solid ${who === opt.id ? opt.color + '55' : 'rgba(255,255,255,0.06)'}`, borderRadius: 14, padding: 12, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', textAlign: 'left' }}>
                ...
                <span style={{ fontSize: 13, color: who === opt.id ? '#F1F5F9' : 'rgba(241,245,249,0.6)', fontWeight: who === opt.id ? 700 : 400, flex: 1, textAlign: 'left' }}>{opt.label}</span>
          ...
          {who === 'child' && (
            <div style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.25)', borderRadius: 14, padding: 14, marginBottom: 14 }}>
              ...
              <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.6)', margin: 0, lineHeight: 1.5 }}>What happened to you is not your fault. You did nothing wrong. No one will know you sent this message.</p>
            </div>
          )}
          {who === 'parent' && (
            <div style={{ background: 'rgba(236,72,153,0.07)', border: '1px solid rgba(236,72,153,0.22)', borderRadius: 14, padding: 14, marginBottom: 14 }}>
              ...
              <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.6)', margin: 0, lineHeight: 1.5 }}>Children rarely lie about abuse. Stay calm and listen without judgment. Do not confront the perpetrator yourself.</p>
            </div>
          )}
          {who === 'teacher' && (
            <div style={{ background: 'rgba(16,185,129,0.07)', border: '1px solid rgba(16,185,129,0.22)', borderRadius: 14, padding: 14, marginBottom: 14 }}>
              ...
              <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.6)', margin: 0, lineHeight: 1.5 }}>As school staff you are a mandated reporter. Do not investigate alone — report and let trained authorities take over.</p>
            </div>
          )}
      ...
      {step === 'what' && (
        <div>
          <h2 style={{ color: '#F1F5F9', fontSize: 15, fontWeight: 700, marginBottom: 4 }}>What happened?</h2>
          <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.5)', marginBottom: 14, lineHeight: 1.5 }}>Only share what you are comfortable with. Even a few words help. There are no wrong answers.</p>

          <p style={{ fontSize: 10, color: 'rgba(241,245,249,0.5)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600, marginBottom: 8 }}>Type of incident (select all that apply)</p>
          {INCIDENT_TYPES.map(opt => {
            const selected = incidents.includes(opt.id)
            return (
              <button key={opt.id} onClick={() => toggleIncident(opt.id)}
                style={{ width: '100%', background: selected ? 'rgba(139,92,246,0.08)' : '#111827', border: `1px solid ${selected ? 'rgba(139,92,246,0.4)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 12, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: 6, textAlign: 'left' }}>
                ...
                <span style={{ fontSize: 12, color: selected ? '#F1F5F9' : 'rgba(241,245,249,0.6)', fontWeight: selected ? 600 : 400 }}>{opt.label}</span>
              </button>
            )
          })}

          <p style={{ fontSize: 10, color: 'rgba(241,245,249,0.5)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600, margin: '14px 0 6px' }}>In your own words (optional)</p>
          ...
          <p style={{ fontSize: 10, color: 'rgba(241,245,249,0.5)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600, marginBottom: 8 }}>Who did this? (optional)</p>
          {PERPETRATORS.map(opt => (
            <button key={opt} onClick={() => setPerpetrator(perpetrator === opt ? '' : opt)}
              style={{ width: '100%', background: perpetrator === opt ? 'rgba(139,92,246,0.08)' : '#111827', border: `1px solid ${perpetrator === opt ? 'rgba(139,92,246,0.4)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 12, padding: '9px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', marginBottom: 5 }}>
              <span style={{ fontSize: 12, color: perpetrator === opt ? '#F1F5F9' : 'rgba(241,245,249,0.6)', fontWeight: perpetrator === opt ? 600 : 400 }}>{opt}</span>
      ...
      {step === 'where' && (
        <div>
          <h2 style={{ color: '#F1F5F9', fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Where did this happen?</h2>
          <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.5)', marginBottom: 14, lineHeight: 1.5 }}>This helps connect you to local support and prevents it from happening to other children.</p>

          {LOCATIONS.map(loc => (
            <button key={loc} onClick={() => setLocation(location === loc ? '' : loc)}
              style={{ width: '100%', background: location === loc ? 'rgba(139,92,246,0.08)' : '#111827', border: `1px solid ${location === loc ? 'rgba(139,92,246,0.4)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 12, padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', marginBottom: 7 }}>
              <span style={{ fontSize: 13, color: location === loc ? '#F1F5F9' : 'rgba(241,245,249,0.6)', fontWeight: location === loc ? 600 : 400 }}>{loc}</span>
          ...
          <div style={{ background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 13, padding: 12, marginTop: 6, marginBottom: 16, display: 'flex', gap: 9 }}>
            ...
            <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.5)', margin: 0, lineHeight: 1.5 }}>Only submit when you are in a safe place and no one can see your screen. Use Quick Exit if needed.</p>
          </div>
      ...
      {step === 'support' && (
        <div>
          <h2 style={{ color: '#F1F5F9', fontSize: 15, fontWeight: 700, marginBottom: 4 }}>Support Resources</h2>
          <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.5)', marginBottom: 16, lineHeight: 1.5 }}>These are trusted, trained organizations. Everything you share with them is confidential. They will not judge you.</p>
          {SUPPORT_RESOURCES.map((r, i) => (
            <div key={i} style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: 14, marginBottom: 10 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 3 }}>
                    <p style={{ fontWeight: 700, fontSize: 13, color: '#F1F5F9', margin: 0, flex: 1 }}>{r.name}</p>
                  ...
                  <p style={{ fontSize: 11, color: 'rgba(241,245,249,0.35)', margin: '0 0 6px' }}>{r.hours}</p>
      ...
      {step === 'understand' && (
        <div>
          <h2 style={{ color: '#F1F5F9', fontSize: 15, fontWeight: 700, marginBottom: 16 }}>Understanding Sexual Abuse</h2>
          {UNDERSTAND_QA.map(({ q, a, color }, i) => (
            <div key={i} style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.06)', borderLeft: `3px solid ${color}`, borderRadius: 14, padding: 14, marginBottom: 10 }}>
              <p style={{ fontWeight: 700, fontSize: 13, color: '#F1F5F9', marginBottom: 6 }}>{q}</p>
              <p style={{ fontSize: 12, color: 'rgba(241,245,249,0.6)', margin: 0, lineHeight: 1.6 }}>{a}</p>
            </div>
          ))}
        </div>
      )}
