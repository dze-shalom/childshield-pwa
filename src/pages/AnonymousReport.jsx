...
export default function AnonymousReport() {
  ...
  if (submitted) return (
    <div className="page" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
      ...
      <h2 style={{ color: 'var(--text-primary)', marginBottom: 8 }}>{t('report','received')}</h2>
      <p style={{ color: 'var(--text-secondary)', maxWidth: 240, marginBottom: 24, fontSize: 14 }}>
    ...
    <div className="page">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <h1 style={{ fontWeight: 800, fontSize: 18, color: 'var(--text-primary)', margin: 0 }}>{t('report','title')}</h1>
        </div>
        <button onClick={() => { window.location.href = 'https://www.google.com' }} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'var(--overlay-hover)', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 11, fontWeight: 600 }}>
      ...
      <Link to="/found-child" style={{ textDecoration: 'none', display: 'block', marginBottom: 10 }}>
        <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.35)', borderRadius: 16, padding: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)', margin: '0 0 2px' }}>I Found a Child</p>
            <p style={{ fontSize: 11, color: 'var(--text-secondary)', margin: 0 }}>AI matches your description against active missing alerts instantly</p>
          </div>
      ...
      <Link to="/abuse-report" style={{ textDecoration: 'none', display: 'block', marginBottom: 16 }}>
        <div style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: 16, padding: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)', margin: '0 0 2px' }}>{t('report','sexual')}</p>
            <p style={{ fontSize: 11, color: 'var(--text-secondary)', margin: 0 }}>{t('report','sexualSub')}</p>
          </div>
      ...
      <div className="card" style={{ display: 'flex', gap: 10, marginBottom: 16, padding: 12, background: 'rgba(59,130,246,0.06)', borderColor: 'rgba(59,130,246,0.2)' }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: 0 }}>{t('report','anonymous')}</p>
      </div>

      <p style={{ fontSize: 10, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600, marginBottom: 8 }}>{t('report','type')}</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
        {INCIDENT_TYPES.map((t) => (
          <button key={t.value} onClick={() => update('type', t.value)}
            style={{ background: form.type === t.value ? 'rgba(239,68,68,0.07)' : '#111827', border: `1px solid ${form.type === t.value ? 'rgba(239,68,68,0.4)' : 'var(--border)'}`, borderRadius: 12, padding: '11px 12px', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', textAlign: 'left' }}>
            <div style={{ width: 30, height: 30, background: form.type === t.value ? 'rgba(239,68,68,0.15)' : 'var(--border)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <t.icon size={14} color={form.type === t.value ? '#EF4444' : 'var(--text-muted)'} />
            </div>
            <div style={{ flex: 1 }}>
              <p style={{ fontWeight: form.type === t.value ? 700 : 400, fontSize: 13, color: form.type === t.value ? 'var(--text-primary)' : 'var(--text-secondary)', margin: 0 }}>{t.label}</p>
      ...
      <p style={{ fontSize: 10, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600, marginBottom: 6 }}>{t('report','location')}</p>
      ...
      <p style={{ fontSize: 10, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600, marginBottom: 6 }}>{t('report','what')}</p>
      ...
      <p style={{ textAlign: 'center', fontSize: 11, color: 'var(--text-placeholder)', marginTop: 10 }}>
        {t('report','note')}
      </p>
    </div>
  )
