...
import { Bell, Shield, AlertCircle, CheckCircle2, Users, Clock, MessageSquare, AlertTriangle, LifeBuoy, Lock, Sun, Moon } from 'lucide-react'
    ...
    <div className="page">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <h1 style={{ fontWeight: 800, fontSize: 20, color: 'var(--text-primary)', margin: 0 }}>ChildShield</h1>
            <p style={{ fontSize: 10, color: 'var(--text-muted)', margin: 0 }}>Cameroon</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {/* Theme toggle — visible on mobile only (desktop has it in the sidebar) */}
          <button
            onClick={toggleTheme}
            className="md:hidden"
            style={{ width: 38, height: 38, background: 'var(--overlay-hover)', border: 'none', borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--text-muted)' }}
          >
            {isDark ? <Sun size={17} /> : <Moon size={17} />}
          </button>
          <div style={{ position: 'relative' }}>
            <button style={{ width: 38, height: 38, background: 'var(--overlay-hover)', border: 'none', borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, background: 'var(--overlay-hover)', border: '1px solid var(--border)', borderRadius: 12, padding: '10px 14px' }}>
        <Globe size={14} color="var(--text-muted)" />
        <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600, flex: 1 }}>Language</span>
        <LanguageSwitcher />
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
        {[
          ...
          <div key={label} className="card" style={{ flex: 1, textAlign: 'center', padding: 12 }}>
            ...
            <p style={{ fontSize: 10, color: 'var(--text-muted)', margin: '2px 0 0' }}>{label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, padding: '12px 14px' }}>
        ...
        <div>
          <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>{t('home', 'avgResponse')}</p>
      ...
      <Link to="/childvoice" style={{ textDecoration: 'none', display: 'block', marginBottom: 16 }}>
        <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: 'rgba(18,140,126,0.07)', borderColor: 'rgba(18,140,126,0.22)' }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)', margin: 0 }}>ChildVoice WhatsApp Bot</p>
            <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '2px 0 0' }}>2 reports received today · {t('home', 'botOnline')}</p>
          </div>
        ...
        <section style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              ...
              <h2 style={{ fontWeight: 800, fontSize: 13, color: 'var(--text-primary)', margin: 0 }}>{t('home', 'activeAlerts')}</h2>
            </div>
        ...
        <section style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              ...
              <h2 style={{ fontWeight: 800, fontSize: 13, color: 'var(--text-primary)', margin: 0 }}>
      ...
      {resolvedAlerts.length > 0 && (
        <section style={{ marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
            ...
            <h2 style={{ fontWeight: 700, fontSize: 11, color: 'var(--text-muted)', margin: 0, textTransform: 'uppercase', letterSpacing: 1 }}>{t('home', 'recentlyFound')}</h2>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {/* Quick Actions */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
        <h2 style={{ fontWeight: 700, fontSize: 11, color: 'var(--text-muted)', margin: 0, textTransform: 'uppercase', letterSpacing: 1 }}>{t('home', 'quickActions')}</h2>
      </div>
        ...
        {[
          ...
          <Link key={to} to={to} style={{ flex: 1, textDecoration: 'none' }}>
            <div className="card" style={{ padding: 12, background: cardBg }}>
              ...
              <p style={{ fontWeight: 700, fontSize: 12, color: 'var(--text-primary)', margin: '0 0 2px' }}>{label}</p>
